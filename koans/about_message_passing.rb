...
class AboutMessagePassing < Neo::Koan
  ...
  def test_methods_can_be_invoked_more_dynamically
    mc = MessageCatcher.new
    assert mc.send("caught?")
    assert mc.send("caught" + "?")    # Adding "?" to complete the method name
    assert mc.send("CAUGHT?".downcase)  # Convert the string to lowercase
  end
  ...
  def test_send_with_underscores_will_also_send_messages
    ...
    # THINK ABOUT IT:
    #
    # Ruby provides both `send` and `__send__` to avoid conflicts if a class has a method named `send`.
  end
  ...
  def test_calling_method_missing_causes_the_no_method_error
    ...
    # THINK ABOUT IT:
    #
    # Redefining `method_missing` can capture undefined methods. However, if `method_missing` itself is not defined or falls back to `super`, Ruby raises a `NoMethodError`.
  end
